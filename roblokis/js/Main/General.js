...
/* globals BROWSER, escapeHTML, escapeJSON */
	...
	...{
		...
		database: {
			...
			async load() {
				...
				if (typeof data === "undefined")
					data = null;
				else if (data != null && JSON.stringify(data) === "{}")
					data = null;

				if (data !== null && typeof data.Roblokis !== "undefined")
					data = data.Roblokis;
				...
				function loadFromOldDatabase() {
					let oldData = null;
					const rawOldData = localStorage.getItem("Roblokis");
					if (rawOldData) {
						try {
		...
		GetSettingValue(setting) {
			...
			if (typeof setting == "string")
				setting = Rkis.wholeData[setting];
		...
		GetSettingCustomization(setting) {
			...
			if (setting == null || typeof setting.id != "string")
				return null;
		...
		GetSettingDetails(details) {
			...
			if (details[Rkis.languageCode] != null)
				code = Rkis.languageCode;

			return details[code];
		},
		IsSettingEnabled(setting, defaultSetting, callback) {
			if (typeof defaultSetting == "function")
				[callback, defaultSetting] = [defaultSetting, null];
			...
			if (defaultSetting != null) {
				if (rksetting && rksetting.id == null) {
					...
					if (typeof rksetting == "boolean")
						valueObject.switch = Rkis.wholeData[setting];
					else if (typeof rksetting == "string")
						valueObject.text = Rkis.wholeData[setting];
			...
			if (rksetting && rksetting.options && rksetting.options.disabled === true)
				return false;

			const value = Rkis.GetSettingValue(rksetting);
			let result = true;
		...
		CopyText(text) {
			...
			try {
				...
				const msg = successful ? Rkis.language.copyTextSuccess : Rkis.language.copyTextUnseccuss;
				Rkis.Toast(msg);
			}
			catch (err) {
		...
		Toast(text, ms) {
			...
			setTimeout(() => {
				Rkis.ToastHolder.style.opacity = "0";
				Rkis.ToastHolder.style.bottom = "0px";
			}, ms || 4000);
		},
		/**
		...
		versionCompare(a, b) {
			...
			const aParts = a.split(".").map(x => Number.parseInt(x));
			const bParts = b.split(".").map(x => Number.parseInt(x));
...
else if (window.location.href.includes(".com/my/avatar")) {
	Rkis.pageName = "avatarpage";
}
else if (window.location.href.includes(".com/home")) {
	Rkis.pageName = "home";
}
else if (window.location.href.includes(".com/catalog")) {
	Rkis.pageName = "catalog";
}
else {
	Rkis.pageName = "all";
}
...
(async function () {
	...
	stng.addEventListener("click", async () => {
		...
		if (doc == null || doc.querySelector(".roblokis-settings-button") != null)
			return;
...
(async function () {
	...
	const defaultSettings = await BROWSER.runtime.sendMessage({ about: "getURLRequest", url: "https://ameerdotexe.github.io/roblokis/data/settings/default.json" }).catch(() => null);
	...
	for (const settingName in defaultSettings) {
		const setting = defaultSettings[settingName];
...
databaseLoading.then(() => {
	...
	(function () {
		// future ameer's note: what is this ugly code >:(
		...
		document.$watchLoop("[data-translate]", (e) => {
			const place = e.dataset.translate;
			if (place == null || place === "")
				return;
			...
			valueText = valueText
				.split("$1$")
				.join(e.dataset.translate1)
				.split("$2$")
				.join(e.dataset.translate2)
				.split("$3$")
				.join(e.dataset.translate3)
				.split("$4$")
				.join(e.dataset.translate4)
				.split("$5$")
				.join(e.dataset.translate5)
				.split("$6$")
				.join(e.dataset.translate6)
				.split("$7$")
				.join(e.dataset.translate7)
				.split("$8$")
				.join(e.dataset.translate8)
				.split("$9$")
				.join(e.dataset.translate9);

			valueText = escapeHTML(valueText);

			if (e.tagName === "INPUT")
				e.placeholder = valueText;
			else
				e.textContent = valueText;
		});
				...
				.then((text) => {
					...
					for (const code in allCodes) {
						...
						Rkis.language[code].split("$").forEach((e, i) => {
							if (i === 0)
								return Rkis.language[code] = e;

							if (i % 2 === 1)
								Rkis.language[code] += `$${Math.floor(i / 2) + 1}$`;
							else
								Rkis.language[code] += e;
						});
					}
				})
		...
		const l = (msg) => {
			if (Rkis.language[msg] != null && Rkis.language[msg] !== "")
				return;

			Rkis.languageCode = BROWSER.i18n.getUILanguage().replace("_");

			if (allCodes[msg] != null)
				Rkis.language[msg] = allCodes[msg].message;
			if (Rkis.language[msg] == null)
				Rkis.language[msg] = getLocaleMessage(msg, ["$1$", "$2$", "$3$", "$4$", "$5$", "$6$", "$7$", "$8$", "$9$"]);

			Rkis.language[msg].split("$").forEach((e, i) => {
				if (i === 0)
					return Rkis.language[msg] = e;

				if (i % 2 === 1)
					Rkis.language[msg] += `$${Math.floor(i / 2) + 1}$`;
				else
					Rkis.language[msg] += e;
			});
		...
		(async function () {
			...
			if (Array.isArray(Rkis.wholeData.ExperimentsCSS) && Rkis.wholeData.ExperimentsCSS.length > 0) {
				...
				for (let index = 0; index < Rkis.wholeData.ExperimentsCSS.length; index++) {
					...
					if (experiment == null || typeof experiment.cssUrl != "string")
						continue;
					...
					if (experiment.filterUrls != null) {
						...
						for (let partIndex = 0; partIndex < experiment.filterUrls.length; partIndex++) {
							...
							if (!window.location.href.toLowerCase().includes(urlPart.toLowerCase()))
								continue;
...
document.$watchLoop(".rk-tab", (tab) => {
	if (tab.getAttribute("page") == null)
		return;

	if (tab.dataset.listening === "1")
		return;
	...
	tab.addEventListener("click", () => {
		...
		tabHolder.$findAll(".rk-tab.is-active", (offtab) => {
			offtab.classList.remove("is-active");
		});
		tab.classList.add("is-active");

		pageHolder.$findAll(".rk-tab-page.is-active", (page) => {
			page.classList.remove("is-active");
		});
		pageHolder.$find(`.rk-tab-page[tab="${tab.getAttribute("page")}"]`, (page) => {
...
document.$watchLoop("[data-nav-page]", (tab) => {
	...
	tab.addEventListener("click", () => {
		...
		popupElement.$findAll(".active[data-nav-page]", (offtab) => {
			offtab.classList.remove("active");
		});
		tab.classList.add("active");

		popupElement.$findAll(".active[data-nav-tab]", (page) => {
			page.classList.remove("active");
		});
		popupElement.$find(`[data-nav-tab="${tab.dataset.navPage}"]`, (page) => {
...
function getLocaleMessage(messageName, _substitutions) {
	...
	let substitutions = [];
	if (typeof _substitutions === "object")
		substitutions = _substitutions.slice(0, 8);
	else if (typeof _substitutions === "string")
		substitutions.push(_substitutions);
