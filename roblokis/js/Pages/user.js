...
/* globals BROWSER, $r, escapeHTML, FetchImage */
...
Rkis.StylesList.users = {
	userbanner: {
		imglink: {
			load(style) {
				let bannerGradient = null;
				if (this.bannerGradient != null) {
					bannerGradient = this.bannerGradient;
				}
				else {
					bannerGradient = document.createElement("div");
				}
			...
			unload() {
				if (this.bannerGradient == null)
					return;
				this.bannerGradient.remove();
				this.bannerGradient = null;
			},
		},
	},
};
...
Rkis.page.user = () => {
	...
	if (Rkis.IsSettingEnabled("QuickRemove", {
			...
			fetch(`https://friends.roblox.com/v1/users/${theid}/unfriend`, {
				...
				.then((resp) => {
					...
					// window.location.reload();
						const daitm = document.$find(`#delbtn${theid}`);
						if (daitm)
							daitm.remove();
					}
				})
				.catch((err) => {
					console.error("Couldn't UnFriend the Person");
					console.error(err);
				});
		};

		Rkis.Scripts.QuickFriendRemove.setup = function () {
			document.$watch("div.tab-content.rbx-tab-content > div.tab-pane.active > div.friends-content.section", () => {
				return $r("#friends > a").classList.contains("active") || false;
			}, () => { }).$then().$watchLoop("ul.avatar-cards > li.avatar-card", (e) => {
				const placetoadd = e.$find("div > div > div.avatar-card-caption > span");
		...
		Rkis.Scripts.QuickFriendRemove.first = async function () {
			...
			const robloxuserid = document.$find("head > meta[data-userid]", e => e.dataset.userid);
	...
	if (Rkis.IsSettingEnabled("CancelPending", {
		...
		Rkis.Scripts.CancelFriendPending.secondone = function () {
			const target = $r(`ul.details-actions > li.btn-friends > button`);
			const theid = Rkis.UserId;
			if (theid == null)
				return;
	...
	if (Rkis.IsSettingEnabled("LastStats", {
		...
		Rkis.Scripts.LastStats = async function () {
			const id = Rkis.UserId;
			if (id == null)
				return;

			function getPresences(url, json) {
				return new Promise((resolve) => {
					BROWSER.runtime.sendMessage({ about: "postURLRequest", url, jsonData: json }, (data) => {
						resolve(data);
					});
				});
			}
			...
			let result = await getPresences(`https://presence.roblox.com/v1/presence/users`, { userIds: [id] });
			...
			.catch(() => {return null;}); */
			if (result != null && result.userPresences != null)
				result = result.userPresences;
			...
			const statusholder = await document.$watch("#profile-statistics-container > .section.profile-statistics").$promise();
			if (statusholder == null)
				return;

			const oldthingytodelete = document.querySelector("#lastseentheprofileis");
			if (oldthingytodelete != null)
				oldthingytodelete.remove();
