...
/* globals $r, escapeHTML, HTMLParser */
	...
	servers: {
		"": {
			...
			update: (style) => {
				...
				if (options == null)
					return;
				document.body.classList.toggle("servers-no-players", options.hidePlayers === true);
			},
			unload: () => {
				document.body.classList.remove("servers-no-players");
			},
		},
		"card": {
...
Rkis.page.game = () => {
	...
	document.$watchLoop(`[class$="'"]`, (e) => {
		const broken_class = e.classList[e.classList.length - 1];
		const fixed_class = broken_class.slice(0, -1);
	...
	if (Rkis.IsSettingEnabled("SmallServer", {
		...
		(function () {
			...
			Rkis.Scripts.SmallServer.createSection = function () {
				...
				if (Rkis.Scripts.SmallServer.running === true)
					return;
				...
				if (smallrunninggames == null) {
					...
					document.querySelector("#running-game-instances-container")
						.insertBefore(smallrunninggames, document.querySelector("#rbx-public-running-games"));
				}
				...
				smallrunninggames.$find("ul", (e) => {
					if (e.querySelector("span.spinner") != null)
						return;
					e.innerHTML = `<span class="spinner spinner-default"></span>`;
				});

				const placeId = Rkis.GameId;
			...
			Rkis.Scripts.SmallServer.showServers = async function (PlaceId, servers) {
				...
				const gameinstances = document.querySelector("#running-game-instances-container");
				if (!gameinstances)
					return;

				const smallrunninggames = document.querySelector("#rbx-small-running-games");
				if (!smallrunninggames)
					return Rkis.Scripts.SmallServer.createSection();

				smallrunninggames.$findAll("button", (e) => {
					e.disabled = false;
				});

				const smallservers = smallrunninggames.$find("ul", (e) => {
					if (e.querySelector("span.spinner") == null)
						return e;
					e.$clear();
					return e;
				...
				const thumbnailsToFetch = [];
				const loadmorelists = [];
				const hasLink = Rkis.IsSettingEnabled("SmallServerLink", {
					id: "SmallServerLink",
					type: "switch",
					value: { switch: true },
					details: {
				...
				for (let i = 0; i < servers.length; i++) {
					if (servers[i].playing <= 0)
						continue;

					const smallserverHolder = document.createElement("li");
					smallserverHolder.className = "rbx-small-game-server-item col-md-3 col-sm-4 col-xs-6";
					const smallserver = HTMLParser("<div class=\"card-item\">");
					smallserverHolder.appendChild(smallserver);

					const smallserverplayers = document.createElement("div");
					...
					for (let o = 0; o < ROBLOX_ICON_LIMIT && o < servers[i].playerTokens.length; o++) {
						...
						smallserverplayers.append(
							HTMLParser("<span class=\"avatar avatar-headshot-md player-avatar\">", HTMLParser("<span class=\"thumbnail-2d-container avatar-card-image\">", HTMLParser(`<img data-thumbnailrequestid="${escapeHTML(thumbnail.requestId)}">`))),
						);

						thumbnailsToFetch.push(thumbnail);
					}
					if (servers[i].playerTokens.length > ROBLOX_ICON_LIMIT) {
						const differenceMore = servers[i].playerTokens.length - ROBLOX_ICON_LIMIT;

						smallserverplayers.append(
							HTMLParser("<span class=\"avatar avatar-headshot-md player-avatar hidden-players-placeholder\">", `+${differenceMore}`),
						);
					}
					...
					if (lineWidth > 100)
						lineWidth = 100;
					const smallserverdetails = HTMLParser("<div class=\"rbx-game-server-details game-server-details\">", HTMLParser("<div class=\"text-info rbx-game-status rbx-game-server-status text-overflow\">", `${escapeHTML(servers[i].playing)} / ${escapeHTML(servers[i].maxPlayers)}`), HTMLParser("<div class=\"server-player-count-gauge border\">", HTMLParser(`<div class="gauge-inner-bar border" style="width: ${escapeHTML(lineWidth)}%;">`)));
					...
					if (hasLink) {
						smallserverdetails.append(
							HTMLParser(`<span data-placeid="${escapeHTML(PlaceId)}" class="rk-multi-button">`, HTMLParser(`<a class="btn-control-xs rk-copy-link-btn" data-placeid="${escapeHTML(PlaceId)}" data-serverid="${escapeHTML(servers[i].id)}">`, "🔗"), HTMLParser(`<button type="button" class="btn-full-width btn-control-xs rbx-game-server-join game-server-join-btn btn-primary-md btn-min-width" onclick="Roblox.GameLauncher.joinGameInstance(${escapeHTML(PlaceId)}, '${escapeHTML(servers[i].id)}');">`, Rkis.language.joinButtons,
							)),
						);
					}
					else {
						smallserverdetails.append(
							HTMLParser(`<span data-placeid="${escapeHTML(PlaceId)}">`, HTMLParser(`<button type="button" class="btn-full-width btn-control-xs rbx-game-server-join game-server-join-btn btn-primary-md btn-min-width" onclick="Roblox.GameLauncher.joinGameInstance(${escapeHTML(PlaceId)}, '${escapeHTML(servers[i].id)}');">`, Rkis.language.joinButtons,
							)),
						);
					}
					...
					if (i < ROBLOX_SERVERS_LIMIT)
						smallservers.append(smallserverHolder);
				...
				for (let i = 0; i < loadmorelists.length; i += ROBLOX_SERVERS_LIMIT) {
					...
					if (i / ROBLOX_SERVERS_LIMIT !== 0) {
						loadmorebutton.style.display = "none";
						loadmorebutton.dataset.loadmore = i / ROBLOX_SERVERS_LIMIT - 1;
					}
					...
					loadmorebutton.addEventListener("click", (event) => {
						if (event.target.tagName !== "BUTTON"
							...
							|| event.target.dataset.loadnum === "") {
							return;
						}

						const selector = `[data-loadmore="${event.target.dataset.loadnum}"]`;
						const list = smallrunninggames.querySelectorAll(selector);
					...
					smallrunninggames.querySelector(".rbx-public-running-games-footer")?.append(loadmorebutton);
				...
				for (let i = 0; i < thumbnailsToFetch.length; i += 99) {
					fetch("https://thumbnails.roblox.com/v1/batch", {
						...
						.then((list) => {
							list.data.forEach((e) => {
								const elem = document.querySelector(`img[data-thumbnailrequestid="${e.requestId}"]`);
								if (elem == null)
									return;
	...
	if (Rkis.IsSettingEnabled("PageNav", {
		...
		(function () {
			...
			Rkis.Scripts.PageNav.firstone = async function () {
				const result = await fetch(`https://games.roblox.com/v1/games/${Rkis.GameId}/servers/Public?limit=100&sortOrder=Desc&excludeFullGames=true`)
					.then(response => response.json())
					.then((data) => {
						if (data.data == null)
							return null;
				...
				let buttonplace = await document.$watch("#rbx-public-running-games > div.rbx-public-running-games-footer").$promise();
				if (buttonplace == null)
					return;
				buttonplace.style.display = "none";

				document.$find("#rbx-public-running-games").innerHTML += `<div class="rbx-running-games-footer roblokis-footer"><button type="button" class="btn-control-sm btn-full-width rbx-running-games-load-more">${Rkis.language.loadMore}</button></div>`;
				buttonplace = document.$find("#rbx-public-running-games > div.rbx-public-running-games-footer.roblokis-footer");

				const pagecount = Math.floor((Rkis.Scripts.PageNav.totalServers.length || 0) / 10) + 1;
				...
				document.$find("#rbx-public-game-server-item-container", e => e.classList.add("pagenav"));
				...
				document.$find("#rbx-public-running-games > div.btr-pager-holder.btr-server-pager", e => e.remove());
			};

			Rkis.Scripts.PageNav.getpage = async function (pagenum, more) {
				if (Rkis.Scripts.PageNav.running === true)
					return;
				...
				let pagecount = Math.floor((Rkis.Scripts.PageNav.totalServers.length || 0) / 10) + 1;
				if (pagenum > pagecount)
					pagenum = pagecount;

				const pagenav = document.$find("#rkpagenav");
				if (pagenav)
					pagenav.dataset.page = pagenum;

				const pagenavnum = document.$find("#rkpagenavnum");
				if (pagenavnum)
					pagenavnum.value = pagenum;

				if (pagenum >= pagecount) {
					const result = await fetch(`https://games.roblox.com/v1/games/${Rkis.GameId}/servers/Public?limit=100&sortOrder=Desc&cursor=${Rkis.Scripts.PageNav.nextPage || ""}&excludeFullGames=true`)
						.then(response => response.json())
						.then((data) => {
							if (data.data == null)
								return null;
				...
				if (pagenav)
					pagenav.dataset.max = pagecount;
				let hasMore = Rkis.Scripts.PageNav.nextPage !== "";
				if (hasMore)
					hasMore = pagenav.dataset.max !== `${pagenum}`;
				if (pagenav)
					pagenav.$find("span > span").textContent = `/ ${pagenav.dataset.max}${hasMore ? "+" : ""}`;

				const serversToAdd = [];
				for (let o = pagenum * 10 - 10; o < pagenum * 10 && o < Rkis.Scripts.PageNav.totalServers.length; o++) {
					serversToAdd.push(Rkis.Scripts.PageNav.totalServers[o]);
				}
			...
			Rkis.Scripts.PageNav.next = function (more) {
				const pagenav = document.$find("#rkpagenav");
				if (pagenav == null || pagenav.dataset.page == null)
					return;

				let num = Number(pagenav.dataset.page) + 1;
				if (num < 1)
					num = 1;
				if (num > pagenav.dataset.max)
					num = pagenav.dataset.max;
			...
			Rkis.Scripts.PageNav.last = function () {
				...
				const num = Number(pagenav.dataset.max);

				Rkis.Scripts.PageNav.getpage(num);
			};

			Rkis.Scripts.PageNav.setpage = function (data, more) {
				const holder = document.$find("#rbx-public-game-server-item-container");
				...
				const useLimit = Rkis.IsSettingDisabled("UseThemes");
				...
				for (let i = 0; i < data.length; i++) {
					...
					for (let o = 0; o < server.playerTokens.length; o++) {
						if (useLimit !== false && o >= ROBLOX_ICON_LIMIT)
							break;
					...
					fetch("https://thumbnails.roblox.com/v1/batch", {
						...
						.then((list) => {
							if (list.data == null)
								return null;
							list.data.forEach((e) => {
								const elems = document.querySelectorAll(`img[data-thumbnailrequestid="${e.requestId}"]`);
								if (elems == null)
									return;
	...
	if (Rkis.IsSettingEnabled("PublicServersLink", {
		...
		(function () {
			...
			// document.$watchLoop("li.rbx-public-game-server-item[data-gameid]", (server) => {
			...
			document.$watch("#rbx-public-running-games", (publicServerContainer) => {
			...
			Rkis.Scripts.PublicServersLink.loadMore = async function (placeId, options) {
				if (Rkis.Scripts.PublicServersLink.isOn === false)
					return;
				Rkis.Scripts.PublicServersLink.isOn = false;

				document.$watch("#rbx-public-game-server-item-container .rbx-public-game-server-item *:not(.rk-multi-button) > .game-server-join-btn", () => {
					Rkis.Scripts.PublicServersLink.setupLinks(placeId);
				});

				const payload = [];
				if (options.sortOrder != null)
					payload.push(`sortOrder=${options.sortOrder}`);
				if (options.excludeFullGames != null)
					payload.push(`excludeFullGames=${options.excludeFullGames}`);
				if (options.cursor != null)
					payload.push(`cursor=${options.cursor}`);
				else payload.push(`cursor=${Rkis.Scripts.PublicServersLink.cursor}`);

				const result = await fetch(`https://games.roblox.com/v1/games/${placeId}/servers/Public?${payload.join("&")}`)
					.then(response => response.json())
					.catch(() => null);
			...
			Rkis.Scripts.PublicServersLink.setupLinks = function (placeId) {
				const noLinkPublicServers = document.querySelectorAll("#rbx-public-game-server-item-container .rbx-public-game-server-item *:not(.rk-multi-button) > .game-server-join-btn");

				noLinkPublicServers.forEach((joinButton) => {
					const server = Rkis.Scripts.PublicServersLink.loadedServers[0];
					if (server == null)
						return;
			...
			Rkis.Scripts.PublicServersLink.addButton = function (joinButton, serverId, placeId) {
				const newbtnexist = document.querySelector(`[data-serverid="${escapeHTML(serverId)}"]`);
				if (newbtnexist)
					return;
				...
				const link = `https://${Rkis.SubDomain}.roblox.com/home?placeid=${placeId}&gameid=${serverId}`;
	...
	if (Rkis.IsSettingEnabled("ShowMaxPlayers", {
		...
		document.$watch("#game-instances", () => {
			document.$watch("ul.game-stats-container > li > p.text-label", check => check.textContent.toLowerCase() === "server size", (label) => {
				const playercount = label.parentElement.children[1];

				Rkis.gamePlayers = Number.parseInt(playercount.textContent);
				if (Number.isNaN(Rkis.gamePlayers))
					Rkis.gamePlayers = undefined;

				// if (!(isNaN(Rkis.gamePlayers) !== true && Rkis.gamePlayers < 10)) {
				// 	event.classList.add("max-players-text");
				// }
			});
		});
	}
	...
	if (Rkis.IsSettingEnabled("Badges", {
		...
		(function () {
			...
			const customization = Rkis.GetSettingCustomization("Badges");
			...
			Rkis.Scripts.BadgesView.firstone = async function () {
				const universe = document.$find("#game-detail-meta-data", e => e.dataset.universeId);
				if (universe == null)
					return;

				const results = await fetch(`https://badges.roblox.com/v1/universes/${universe}/badges?cursor=&limit=100&sortOrder=Asc`)
					.then(res => res.json());
				if (results == null)
					return;
				...
				const requestsforimges = [];
				const idsforbadges = [];
				let awardedBadges = null;

				for (let i = 0; i < results.data.length; i++) {
					const badge = results.data[i];
				...
				if (Rkis.wholeData.Designer == null || Rkis.wholeData.Designer.wholeData == null || Rkis.wholeData.Designer.wholeData.BadgesAwarded !== false) {
					awardedBadges = await fetch(`https://badges.roblox.com/v1/users/${document.$find("head > meta[data-userid]", e => e.dataset.userid)}/badges/awarded-dates?badgeIds=${idsforbadges}`)
						.then(response => response.json())
						.catch(() => { return null; });
				}

				const badgeImg = await fetch(`https://thumbnails.roblox.com/v1/batch`, {
					method: "POST",
					headers: {
						"Content-Type": "application/json",
					},
				...
				const badgessection = await document.$watch(".rbx-body .game-badges-list > ul.stack-list").$promise();
				if (badgessection == null)
					return console.error("didn't find badge place");
				...
				for (let x = 0; x < 2; x++) {
					for (let i = 0; i < results.data.length; i++) {
						...
						let badgeawr;
						if (awardedBadges != null && awardedBadges.data != null)
							badgeawr = awardedBadges.data.find(x => x.badgeId === badge.id);

						let creat = null;
						let updat = null;

						if (Rkis.wholeData.Designer == null || Rkis.wholeData.Designer.wholeData == null || Rkis.wholeData.Designer.wholeData.BadgesCreated !== false) {
							creat = new Date(badge.created);
							thecut = true;
						}
						if (Rkis.wholeData.Designer == null || Rkis.wholeData.Designer.wholeData == null || Rkis.wholeData.Designer.wholeData.BadgesUpdated !== false) {
							updat = new Date(badge.updated);
							thecut = true;
						}

						let badgeawrd = "";
						if (badgeawr != null) {
							const award = new Date(badgeawr.awardedDate);
						...
						if (Rkis.Designer.currentTheme?.styles?.badges !== "card")
							thecut = false;

						if (badgeimg?.imageUrl != null)
							mainelement.innerHTML = `<div class="badge-image"> <a href="${window.location.origin}/badges/${escapeHTML(badge.id)}/${escapeHTML(badge.name)}"> <img src="${escapeHTML(badgeimg.imageUrl)}"></a> </div>`;
	...
	if (Rkis.IsSettingEnabled("AvailPrivateServers", {
		...
		document.$watchLoop("#rbx-private-running-games > ul > li", (server) => {
			const jionBtn = server.querySelector("div.rbx-private-game-server-details > span > button.rbx-private-game-server-join");
	...
	if (false && Rkis.wholeData.UseThemes !== false) {
		...
		Rkis.Scripts.ServerPlayerCounterLoader = function (serversitm) {
			const rightsection = serversitm.$find("div.player-thumbnails-container");
			if (!rightsection)
				return;
			if (rightsection.$find("span.rk-plr-counter"))
				return;

			const players = serversitm.$findAll("div.player-thumbnails-container > span:not(.hidden-players-placeholder)");
			if (players.length < 1)
				return;

			const hiddenPlayers = serversitm.$find("div.player-thumbnails-container > span.hidden-players-placeholder");
			...
			let playercount = Rkis.gamePlayers;
			if (playercount == null)
				playercount = document.$find("ul.game-stats-container > li:nth-child(6) > p.text-lead.font-caption-body")?.textContent;
			if (playercount == null)
				playercount = $r("#game-detail-page > div.btr-game-main-container.section-content > div.remove-panel.btr-description > ul > li:nth-child(6) > p.text-lead.font-caption-body")?.textContent;

			if (playercount && Number.parseInt(playercount) <= totalPlayerCount)
				stylee += "background-color: darkred;color: white;";
			else if (playercount && Number.parseInt(playercount) / 2 <= totalPlayerCount)
				stylee += "background-color: orangered;color: white;";
		...
		Rkis.Scripts.PrivateServersView.secondone = function (serversitm) {
			...
			const serverNameElement = serversitm.$find("div.rbx-private-game-server-details > div.section-header > span");
			if (serverNameElement != null)
				serverNameElement.setAttribute("title", serverNameElement.textContent);
		...
		Rkis.Scripts.FriendsServersView.secondone = function (serversitm) {
			...
			const leftsection = serversitm.$find("div.game-server-details");
			if (leftsection == null)
				return;
		...
		document.$watch("#rbx-public-game-server-item-container", () => {
			document.$watchLoop("#rbx-public-game-server-item-container > li", (serverslist) => {
